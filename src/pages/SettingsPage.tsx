import { Bot, Bookmark, Check, Database, LockKeyhole, Save, UserRound } from 'lucide-react'
import { FormEvent, useEffect, useState } from 'react'
import { Link } from 'react-router-dom'
import {
  getAiPreferences, getDatasetNote, listBookmarks, saveAiPreferences, saveDatasetNote,
  type AiPreferences, type Analysis, type ApiDataset, type Bookmark as BookmarkRecord, type DatasetAnalytics,
} from '../api'
...
export function SettingsPage({ dataset, analytics, analysis }: { dataset: ApiDataset | null; analytics: DatasetAnalytics | null; analysis: Analysis | null }) {
  ...
  useEffect(() => {
    let active = true
    Promise.all([getAiPreferences(), listBookmarks(), dataset ? getDatasetNote(dataset.id) : Promise.resolve({ note: { note: '', updated_at: null } })])
      .then(([preferenceResult, bookmarkResult, noteResult]) => { if (active) { setPreferences(preferenceResult.preferences); setBookmarks(bookmarkResult.bookmarks); setNote(noteResult.note.note) } })
